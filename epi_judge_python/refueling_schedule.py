...
def find_ample_city(gallons: List[int], distances: List[int]) -> int:
    remainining_gallons = 0
    CityAndRemainingGas = collections.namedtuple('CityAndRemainingGas', ('city', 'remainining_gallons'))
    city_remaining_gallons_pair = CityAndRemainingGas(0, 0)
    num_cities = len(gallons)
    for i in range(1,num_cities):
        remainining_gallons += gallons[i-1]-distances[i-1]//MPG
        if remainining_gallons < city_remaining_gallons_pair.remainining_gallons:
            city_remaining_gallons_pair = CityAndRemainingGas(i,remainining_gallons)
